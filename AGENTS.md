...
### Gap 6: user-audience tools don't bridge to TUI slash commands

**Status:** Open
**Description:** User-audience tools (registered in the JSX tree via
`createTool({ audience: "user" })`) and TUI `SlashCommand`s duplicate metadata
— name, description, aliases. A user-audience tool like `add-dir` must be defined
as a `createTool` in the agent AND separately wired as a slash command in the
TUI, with the TUI handler calling `accessor.dispatch()`. The shapes are
close but not identical: SlashCommands take raw `args: string`, tools take
structured `input: ZodSchema`.
**Desired:** A `useUserTools({ accessor })` hook in `@agentick/tui` that
introspects the session's user-audience tools and auto-generates `SlashCommand`
entries from them. The bridge would: query session for user-audience tool metadata,
generate SlashCommands where handler calls `dispatch`, parse `args` string
...
correctly NOT user-audience tools — they stay as plain SlashCommands.
